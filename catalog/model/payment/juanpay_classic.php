...
class ModelPaymentJuanPayClassic extends Model {
    ...
    public function call($data) {
        ...
        if ($this->config->get('juanpay_classic_test') == 1) {
            $api_endpoint = 'http://localhost:3000/nvp';
        } else {
    ...
    public function getMethod($address, $total) {
        ...
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('juanpay_classic_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");

        if ($this->config->get('juanpay_classic_total') > $total) {
            $status = false;
        } elseif (!$this->config->get('juanpay_classic_geo_zone_id')) {
            $status = true;
        ...
        if ($status) {
            $method_data = array(
                'code'       => 'juanpay_classic',
                'title'      => $this->language->get('text_title'),
                'sort_order' => $this->config->get('juanpay_classic_sort_order')
            );
        }
    ...
    public function addOrder($order_data) {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "juanpay_order` SET
    ...
    public function addTransaction($transaction_data) {
        /**
         * 1 to many relationship with juanpay order table, many transactions per 1 order
         */

        $this->db->query("INSERT INTO `" . DB_PREFIX . "juanpay_order_transaction` SET
            `juanpay_order_id` = '".(int)$transaction_data['juanpay_order_id']."',
    ...
    public function paymentRequestInfo() {
        ...
        $data['PAYMENTREQUEST_0_PAYMENTACTION'] = $this->config->get('juanpay_classic_method');
    ...
    public function getTransactionRow($transaction_id) {
        $qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "juanpay_order_transaction` `pt` LEFT JOIN `" . DB_PREFIX . "juanpay_order` `po` ON `pt`.`juanpay_order_id` = `po`.`juanpay_order_id`  WHERE `pt`.`transaction_id` = '" . $this->db->escape($transaction_id) . "' LIMIT 1");
    ...
    public function totalCaptured($juanpay_order_id) {
        $qry = $this->db->query("SELECT SUM(`amount`) AS `amount` FROM `" . DB_PREFIX . "juanpay_order_transaction` WHERE `juanpay_order_id` = '" . (int) $juanpay_order_id . "' AND `pending_reason` != 'authorization' AND `pending_reason` != 'paymentreview' AND (`payment_status` = 'Partially-Refunded' OR `payment_status` = 'Completed' OR `payment_status` = 'Pending') AND `transaction_entity` = 'payment'");
