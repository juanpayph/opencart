...
class ControllerPaymentJuanPayClassic extends Controller {
    protected function index() {
        $this->language->load('payment/juanpay_classic');

        $this->data['button_continue'] = $this->language->get('button_continue');
        $this->data['button_continue_action'] = $this->url->link('payment/juanpay_classic/checkout', '', 'SSL');

        /**
         * if there is any other juanpay session data, clear it
         */
        unset($this->session->data['juanpay']);


        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/juanpay_classic.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/juanpay_classic.tpl';
        } else {
            $this->template = 'default/template/payment/juanpay_classic.tpl';
        }
    ...
    public function express() {
        ...
        if($this->customer->isLogged()) {
             ...
             */
            $this->session->data['juanpay']['guest'] = false;
            ...
            if($this->config->get('config_guest_checkout') && !$this->config->get('config_customer_price') && !$this->cart->hasDownload() && !$this->cart->hasRecurringProducts()) {
                 ...
                 */
                $this->session->data['juanpay']['guest'] = true;
            } else {
                /**
        ...
        $data = array(
            'METHOD' => 'SetClassicCheckout',
            'MAXAMT' => $max_amount,
            'RETURNURL' => $this->url->link('payment/juanpay_classic/expressReturn', '', 'SSL'),
            'CANCELURL' => $this->url->link('checkout/cart'),
            'REQCONFIRMSHIPPING' => 0,
            'NOSHIPPING' => $shipping,
            'ALLOWNOTE' => $this->config->get('juanpay_classic_allow_note'),
            'LOCALECODE' => 'EN',
            'LANDINGPAGE' => 'Login',
            'HDRIMG' => $this->model_tool_image->resize($this->config->get('juanpay_classic_logo'), 790, 90),
            'HDRBORDERCOLOR' => $this->config->get('juanpay_classic_border_colour'),
            'HDRBACKCOLOR' => $this->config->get('juanpay_classic_header_colour'),
            'PAYFLOWCOLOR' => $this->config->get('juanpay_classic_page_colour'),
            'CHANNELTYPE' => 'Merchant',
        );

        $data = array_merge($data, $this->model_payment_juanpay_classic->paymentRequestInfo());
        
        $result = $this->model_payment_juanpay_classic->call($data);

        
        
        /**
         * If a failed JuanPay setup happens, handle it.
         */
        if(!isset($result['TOKEN'])) {
             ...
             * If JuanPay debug log is off then still log error to normal error log.
             */
            if($this->config->get('juanpay_classic_debug')) {
        ...
        $this->session->data['juanpay']['token'] = $result['TOKEN'];

        if ($this->config->get('juanpay_classic_test') == 1) {
            header('Location: https://www.sandbox.juanpay.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $result['TOKEN']);
        } else {
            header('Location: https://www.juanpay.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $result['TOKEN']);
        }
    }

    public function expressReturn() {
        /**
         * This is the url when JuanPay has completed the auth.
         *
         * It has no output, instead it sets the data and locates to checkout
         */
        $this->load->model('payment/juanpay_classic');
        $data = array(
            'METHOD' => 'GetClassicCheckoutDetails',
            'TOKEN' => $this->session->data['juanpay']['token'],
        );

        $result = $this->model_payment_juanpay_classic->call($data);
        $this->session->data['juanpay']['payerid']   = $result['PAYERID'];
        $this->session->data['juanpay']['result']    = $result;
        ...
        if($this->session->data['juanpay']['guest'] == true) {
        ...
        $this->redirect($this->url->link('payment/juanpay_classic/expressConfirm', '', 'SSL'));
    }
    ...
    public function expressConfirm() {
        ...
        $this->data['action'] = $this->url->link('payment/juanpay_classic/expressConfirm', '', 'SSL');
        ...
        if($this->cart->hasShipping()) {
            ...
            if (!empty($shipping_address)) {
                ...
                if(!empty($results)) {
                    ...
                    $this->data['action_shipping'] = $this->url->link('payment/juanpay_classic/shipping', '', 'SSL');
                } else {
                    unset($this->session->data['shipping_methods']);
                    unset($this->session->data['shipping_method']);
        ...
        $this->session->data['payment_method'] = $this->session->data['payment_methods']['juanpay_classic'];

        $this->data['action_confirm'] = $this->url->link('payment/juanpay_classic/expressComplete', '', 'SSL');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/juanpay_classic_confirm.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/juanpay_classic_confirm.tpl';
        } else {
            $this->template = 'default/template/payment/juanpay_classic_confirm.tpl';
        }
    ...
    public function expressComplete() {
        ...
        if ($redirect == '') {
            ...
            $juanpay_data = array(
                'TOKEN' => $this->session->data['juanpay']['token'],
                'PAYERID' => $this->session->data['juanpay']['payerid'],
                'METHOD' => 'DoClassicCheckoutPayment',
                'PAYMENTREQUEST_0_NOTIFYURL' => $this->url->link('payment/juanpay_classic/dpn', '', 'SSL'),
                'RETURNFMFDETAILS' => 1,
            );

            $juanpay_data = array_merge($juanpay_data, $this->model_payment_juanpay_classic->paymentRequestInfo());

            $result = $this->model_payment_juanpay_classic->call($juanpay_data);

            if($result['ACK'] == 'Success') {
                //handle order status
                switch($result['PAYMENTINFO_0_PAYMENTSTATUS']) {
                    case 'Canceled_Reversal':
                        $order_status_id = $this->config->get('juanpay_classic_canceled_reversal_status_id');
                        break;
                    case 'Completed':
                         $order_status_id = $this->config->get('juanpay_classic_completed_status_id');
                        break;
                    case 'Denied':
                        $order_status_id = $this->config->get('juanpay_classic_denied_status_id');
                        break;
                    case 'Expired':
                        $order_status_id = $this->config->get('juanpay_classic_expired_status_id');
                        break;
                    case 'Failed':
                        $order_status_id = $this->config->get('juanpay_classic_failed_status_id');
                        break;
                    case 'Pending':
                        $order_status_id = $this->config->get('juanpay_classic_pending_status_id');
                        break;
                    case 'Processed':
                        $order_status_id = $this->config->get('juanpay_classic_processed_status_id');
                        break;
                    case 'Refunded':
                        $order_status_id = $this->config->get('juanpay_classic_refunded_status_id');
                        break;
                    case 'Reversed':
                        $order_status_id = $this->config->get('juanpay_classic_reversed_status_id');
                        break;
                    case 'Voided':
                        $order_status_id = $this->config->get('juanpay_classic_voided_status_id');
                        break;
                }

                $this->model_checkout_order->confirm($order_id, $order_status_id);

                //add order to juanpay table
                $juanpay_order_data = array(
                    'order_id' => $order_id,
                    'capture_status' => ($this->config->get('juanpay_classic_method') == 'Sale' ? 'Complete' : 'NotComplete'),
                ...
                $juanpay_order_id = $this->model_payment_juanpay_classic->addOrder($juanpay_order_data);

                //add transaction to juanpay transaction table
                $juanpay_transaction_data = array(
                    'juanpay_order_id' => $juanpay_order_id,
                    ...
                    'transaction_entity' => ($this->config->get('juanpay_classic_method') == 'Sale' ? 'payment' : 'auth'),
                    'amount' => $result['PAYMENTINFO_0_AMT'],
                    'debug_data' => json_encode($result),
                );                
                $this->model_payment_juanpay_classic->addTransaction($juanpay_transaction_data);
                ...
                if(isset($result['REDIRECTREQUIRED']) && $result['REDIRECTREQUIRED'] == true) { //- handle german redirect here
                    $this->redirect('https://www.juanpay.com/cgi-bin/webscr?cmd=_complete-express-checkout&token='.$this->session->data['juanpay']['token']);
                }
            } else {
                if ($result['L_ERRORCODE0'] == '10486') {
                    if (isset($this->session->data['juanpay_redirect_count'])) {
                        
                        if ($this->session->data['juanpay_redirect_count'] == 2) {
                            $this->session->data['juanpay_redirect_count'] = 0;
                            $this->session->data['error'] = $this->language->get('error_too_many_failures');
                            $this->redirect($this->url->link('checkout/checkout', '', 'SSL'));                            
                        } else {
                            $this->session->data['juanpay_redirect_count']++;
                        }
                    } else {
                        $this->session->data['juanpay_redirect_count'] = 1;
                    }
                    
                    if ($this->config->get('juanpay_classic_test') == 1) {
                        $this->redirect('https://www.sandbox.juanpay.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $this->session->data['juanpay']['token']);
                    } else {
                        $this->redirect('https://www.juanpay.com/cgi-bin/webscr?cmd=_express-checkout&token=' . $this->session->data['juanpay']['token']);
                    }
                }
    ...
    public function checkout() {
        ...
        $data = array(
            'METHOD' => 'SetClassicCheckout',
            'MAXAMT' => $max_amount,
            'RETURNURL' => $this->url->link('payment/juanpay_classic/checkoutReturn', '', 'SSL'),
        ...
        if(!isset($result['TOKEN'])) {            
             ...
             * If JuanPay debug log is off then still log error to normal error log.
             */
            if($this->config->get('juanpay_classic_debug') == 0) {
                $this->log->write(serialize($result));
            }
        ...
        if ($this->config->get('juanpay_classic_test') == 1) {
            header('Location: https://www.sandbox.juanpay.com/cgiâ€‘bin/webscr?cmd=_express-checkout&token=' . $result['TOKEN'].'&useraction=commit');
        } else {
            header('Location: https://www.juanpay.com/cgiâ€‘bin/webscr?cmd=_express-checkout&token=' . $result['TOKEN'].'&useraction=commit');
        }
    }
    ...
    public function dpn() {
        ...
        if ($this->config->get('juanpay_classic_test') == 1) {
            $curl = curl_init('https://www.sandbox.juanpay.com/cgi-bin/webscr');
        } else {
        ...
        if (!$response) {
            $this->model_payment_juanpay_classic->log(array('error' => curl_error($curl),'error_no' => curl_errno($curl)), 'Curl failed');
        }

        $this->model_payment_juanpay_classic->log(array('request' => $request,'response' => $response), 'DPN data');
        ...
        if ( (string)$response == "VERIFIED" )  {
            ...
            if(isset($this->request->post['txn_id'])) {
                $transaction = $this->model_payment_juanpay_classic->getTransactionRow($this->request->post['txn_id']);
            } else {
                $transaction = false;
            }

            if(isset($this->request->post['parent_txn_id'])) {
                $parent_transaction = $this->model_payment_juanpay_classic->getTransactionRow($this->request->post['parent_txn_id']);
            } else {
            ...
            if($transaction) {
                ...
                if($transaction['payment_status'] != $this->request->post['payment_status']) {
                    $this->db->query("UPDATE `" . DB_PREFIX . "juanpay_order_transaction` SET `payment_status` = '" . $this->request->post['payment_status'] . "' WHERE `transaction_id` = '" . $this->db->escape($transaction['transaction_id']) . "' LIMIT 1");
                    ...
                    $this->db->query("UPDATE `" . DB_PREFIX . "juanpay_order_transaction` SET `pending_reason` = '" . $this->request->post['pending_reason'] . "' WHERE `transaction_id` = '" . $this->db->escape($transaction['transaction_id']) . "' LIMIT 1");
                }
            } else {
                ...
                if($parent_transaction) {
                    ...
                    $transaction = array(
                        'juanpay_order_id' => $parent_transaction['juanpay_order_id'],
                    ...
                    if(isset($this->request->post['payment_status']) && $this->request->post['payment_status'] == 'Refunded') {
                        if(($this->request->post['mc_gross'] * -1) == $parent_transaction['amount']) {
                            $this->db->query("UPDATE `" . DB_PREFIX . "juanpay_order_transaction` SET `payment_status` = 'Refunded' WHERE `transaction_id` = '" . $this->db->escape($parent_transaction['transaction_id']) . "' LIMIT 1");
                        } else {
                            $this->db->query("UPDATE `" . DB_PREFIX . "juanpay_order_transaction` SET `payment_status` = 'Partially-Refunded' WHERE `transaction_id` = '" . $this->db->escape($parent_transaction['transaction_id']) . "' LIMIT 1");
                        }
                    }
                    ...
                    if(isset($this->request->post['auth_status']) && $this->request->post['auth_status'] == 'Completed' && $parent_transaction['payment_status'] == 'Pending') {
                        $captured = number_format($this->model_payment_juanpay_classic->totalCaptured($parent_transaction['juanpay_order_id']), 2);
                        $refunded = number_format($this->model_payment_juanpay_classic->totalRefundedOrder($parent_transaction['juanpay_order_id']), 2);
                        ...
                        $this->model_payment_juanpay_classic->updateOrder('Complete', $parent_transaction['order_id']);
                    }
            ...
            $this->model_payment_juanpay_classic->log(array('DPN was invalid'), 'DPN fail');
        } else {
    ...
    public function shipping() {
        ...
        $this->redirect($this->url->link('payment/juanpay_classic/expressConfirm'));
    }
